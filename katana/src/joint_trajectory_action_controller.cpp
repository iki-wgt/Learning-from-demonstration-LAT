			...
			boost::bind(&JointTrajectoryActionController::executeCBFollow, this, _1), false)
{
  ...
  // Benjamin Reiner
  joints_.push_back(katana_->getGripperJointNames().at(0));
  //joints_.push_back(katana_->getGripperJointNames().at(1));
  ...
  {
	ROS_INFO("joint %d: %s", (int)i, joints_[i].c_str());
...
void JointTrajectoryActionController::executeCB(const JTAS::GoalConstPtr &goal)
{
  ...
  ROS_DEBUG("JointTrajectoryActionController received a goal request");
                                                   ...
                                                   boost::function<bool()> isPreemptRequested)
{
  ...
  ROS_DEBUG("stop katana in executeCommon");
  ...
  while (ros::ok())
  {
    ...
    ROS_DEBUG("allJointsReady: %d, allJointsStopped: %d", katana_->allJointsReady(), allJointsStopped());
    // all joints are idle
...
bool JointTrajectoryActionController::validTrajectory(const SpecifiedTrajectory &traj)
{
  ...
  for (double t = traj[0].start_time; t < traj.back().start_time + traj.back().duration; t += 0.01)
  {
    ...
    for (size_t j = 0; j < traj[seg].splines.size(); j++)
    {
      ...
      {
        ROS_ERROR("Velocity %f too high at time %f (joint %zu, segment %d)", vel_t, t, j, seg);
        return false;
      }
